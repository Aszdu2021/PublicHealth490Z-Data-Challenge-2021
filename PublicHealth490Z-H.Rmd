...
CA_all_data <- read_excel("This is Statistics Fall Data Challenge 2021 Dataset_ Food Access Research Atlas Data 2019-ASAFDC 2021.xlsx", sheet = 8)
summary(CA_all_data)
```
...
```
```{r}
###import dataset after delete all null in sheet
dfnoNull <- read_excel("This is no null Statistics Fall Data Challenge 2021 Dataset_ Food Access Research Atlas Data 2019-ASAFDC 2021.xlsx", sheet = 8)
###cor of lasnap1 and 10
cor(dfnoNull$lasnap1, mylist$TractLOWI)
cor(dfnoNull$lasnap10, mylist$TractLOWI)
```
```{r}
df <- CA_all_data %>%
  group_by(LATracts1) %>%
...
df <- CA_all_data %>%
  ...
  ggplot(df, aes(x = "", 
                 ...
                 geom_label_repel(aes(label = labels),
                                  ...
                                  show.legend = FALSE) +
                 coord_polar(theta = "y")
  urban_1mile = 0.23*0.92
  rural_1mile = 0.23*(1-0.92)
  urban_10miles = 0.01*0.92
  rural_10miles = 0.01*(1-0.92)
```
